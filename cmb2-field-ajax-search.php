...
if (class_exists('CMB2_Field_Ajax_Search') === false) {
  ...
  class CMB2_Field_Ajax_Search {
    ...
    public function __construct() {
      add_action('admin_enqueue_scripts', [$this, 'setup_admin_scripts']);

      // Render
      add_action('cmb2_render_post_ajax_search', [$this, 'render'], 10, 5);
      add_action('cmb2_render_user_ajax_search', [$this, 'render'], 10, 5);
      add_action('cmb2_render_term_ajax_search', [$this, 'render'], 10, 5);

      // Display
      add_filter('cmb2_pre_field_display_post_ajax_search', [$this, 'display'], 10, 3);
      add_filter('cmb2_pre_field_display_user_ajax_search', [$this, 'display'], 10, 3);
      add_filter('cmb2_pre_field_display_term_ajax_search', [$this, 'display'], 10, 3);

      // Sanitize
      add_action('cmb2_sanitize_post_ajax_search', [$this, 'sanitize'], 10, 4);
      add_action('cmb2_sanitize_user_ajax_search', [$this, 'sanitize'], 10, 4);
      add_action('cmb2_sanitize_term_ajax_search', [$this, 'sanitize'], 10, 4);

      // Ajax request
      add_action('wp_ajax_cmb_ajax_search_get_results', [$this, 'get_results']);
    }
     ...
     * @return void
     */
    public function render(\CMB2_Field $field, $value, int $object_id, string $object_type, \CMB2_Types $field_type) {
    ...
    /**
     * Display field
     * @param   boolean|mixed       $pre_output Default null value.
     * @param   \CMB2_Field         $field      Field object.
     * @param   \CMB2_Field_Display $display    The `CMB2_Field_Display` object.
     *
     * @return boolean|mixed
     */
    public function display($pre_output, \CMB2_Field $field, \CMB2_Field_Display $display) {
    ...
    public function sanitize($override_value, $value, $object_id, $field_args) {
      $fid = $field_args['id'];
      if (isset($field_args['render_row_cb'][0]->data_to_save[$field_args['id']]) === true) {
        $value = $field_args['render_row_cb'][0]->data_to_save[$field_args['id']];
        if ($field_args['type'] === 'term_ajax_search') {
          if (is_array($value) === true) {
            foreach ($value as $key => $item) {
              $value[$key] = (int)$item;
            }
          }
          wp_set_post_terms($object_id, $value, $fid, false);
        }
      } else {
        $value = false;
      }
    ...
    public function setup_admin_scripts() {
      ...
      wp_register_script('jquery-autocomplete-ajax-search', plugins_url('js/jquery.autocomplete.min.js', __FILE__), ['jquery'], self::VERSION, true);
      wp_register_script(
        'cmb-ajax-search', plugins_url('js/ajax-search.js', __FILE__), [
        'jquery',
      ...
      wp_localize_script(
        'cmb-ajax-search', 'cmb_ajax_search', [
                           'ajaxurl' => admin_url('admin-ajax.php'),
                           'nonce'   => wp_create_nonce('cmb_ajax_search_get_results'),
                           'options' => apply_filters('cmb_field_ajax_search_autocomplete_options', []),
                         ]
      );

      wp_enqueue_script('cmb-ajax-search');
      wp_enqueue_style('cmb-ajax-search', plugins_url('css/ajax-search.css', __FILE__), [], self::VERSION);
    }
     ...
     * @param integer $field_id    The ID of the CMB2_Field
